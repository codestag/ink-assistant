	...
	foreach ( $meta_box['fields'] as $field ) {
		...
		switch ( $field['type'] ) {
			...
			case 'file':
				...
				jQuery(function($){
					var frame,
						isMultiple = "<?php echo esc_js( $multiple ); ?>";
					...
					$('#<?php echo esc_js( $field['id'] ); ?>_button').on('click', function(e) {
						...
						var options = {
							...
							multiple: isMultiple,
							library: {
								type: '<?php echo esc_js( $library_type ); ?>'
							}
						};
			...
			case 'color':
				$default_color = ( ! empty( $field['std'] ) ) ? $field['std'] : '';
				echo '<td class="stag-box-' . $field['type'] . '"><input data-default-color="' . esc_attr( $default_color ) . '" type="text" id="' . $field['id'] . '" name="stag_meta[' . $field['id'] . ']" value="' . ( $meta ? $meta : $field['std'] ) . '" class="colorpicker"></td>';
				?>
				<script type="text/javascript">
					jQuery(document).ready(function($){
						jQuery("#<?php echo esc_js( $field['id'] ); ?>").wpColorPicker();
					});
				</script>
				<?php
				break;
